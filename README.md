...
**Live site:** https://exe1248.github.io/zero.exe/

This repo lives locally on your own machine wherever you cloned it (that
local folder path — `C:\Users\...`, `/home/...`, whatever it is — only means
anything on your computer). The URL above, generated by GitHub Pages once
the repo is pushed, is the actual public link anyone else can open.
...
const CONFIG = {
  ...
  siteMeta: {
    ...
    // Absolute, unlike every other path above — see section 5 below.
    url: "https://your-username.github.io/your-repo/",
    image: "https://your-username.github.io/your-repo/assets/avatar.jpg",
    themeColor: "#0a0a0a",
  },
...
(`exe`) — a slow breathing opacity/blur pulse, plus a bright band that
...
entirely by `config.js -> browserTitle`. For `text: "exe"` that looks like:
`e` → `ex` → `exe` → *pause* → `ex` → `e` → *pause* → repeat.
...
browserTitle: {
  enabled: true,
  text: "exe",          // the string that gets typed/deleted
...
`og:url`, `og:image`) and Twitter Card tags — the preview shown when the
link is pasted into Discord, Twitter/X, iMessage, etc.:

```js
siteMeta: {
  title: "exe",
  description: "building things.",
  // These two are absolute URLs, unlike every other path in config.js —
  // social-preview crawlers fetch og:image externally and can't reliably
  // resolve a page-relative path the way a browser loading the site can.
  url: "https://your-username.github.io/your-repo/",
  image: "https://your-username.github.io/your-repo/assets/avatar.jpg",
  themeColor: "#0a0a0a",
}
...
3. Your link depends on the repo name:
   ...
   This repo is `zero.exe`, so its live link is **https://exe1248.github.io/zero.exe/**.
...
### About getting a link like `guns.lol/exe`
You can't get a path exactly like `https://guns.lol/exe` unless you own or
...
- Buy a **custom domain** (e.g. `exe.xyz`, `exe.dev`, `exe.bio` — cheap
